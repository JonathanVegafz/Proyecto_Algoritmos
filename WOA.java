...
public class WOA {
	...
	private void calcFitness() {
		for (int i = 0; i < population; i++) {
			...
			double fitness = optFunction.calc(positions[i]); // calcula la aptitud para cada agente utilizando la función objetivo.

			// Update the leader
                        //se actualiza la posición del líder si la aptitud del agente actual es mejor que la aptitud del líder actual
			//si se quiere minimizar la funcion objetivo -> si la aptitud del agente actual es menor a la mejor ya encontrada entonces esa sera la nueva mejor encontrada.
                        //si se quiere maximizar la funcion objetivo -> si la aptitud del agente actual es mejor que la mejor ya encontrada esa sera la mejor encontrada.
	...
	private void updatePosition(double a, double a2) {
		...
		for (int i = 1; i < population; i++) {
			...
			double r2 = rand.nextDouble();
                        ...
                        //A y C representan vectores de movimiento.
                        // A y C son valores aleatorios que se utilizan para controlar el movimiento de las ballenas en el algoritmo de optimización de 
                        //ballenas y actualizar su posición en el espacio de búsqueda.
			...
			for (int j = 0; j < dim; j++) {
                            //p representa la probabilidad de elegir entre cazar o actualizar posicion.
                            //p es un numero entre [0,1]
				if (p < 0.5) {
					...
					if (Math.abs(A) < 1) {
						...
						positions[i][j] = randomPos[j] - A * D_X_rand;  // Eq. (2.8)
                                    ...
                                    //calcula la nueva posición de la ballena utilizando la ecuación (2.5) del algoritmo de optimización de ballenas,
                                    //que se utiliza para simular un movimiento hacia el líder
					double distance2Leader = Math.abs(optimalPos[j] - positions[i][j]); //indica la distancia de la i-ésima ballena a la presa

					// Eq. (2.5)
                                        //b y l son valores aleatorios que controlan la amplitud y la dirección del movimiento de la ballena hacia el líder.
	...
	public double[] execute() {
		...
		for (int iter = 0; iter < maxIter; iter++) {
			calcFitness(); //(3) evalua la aptitud de cada ballena de la poblacion actual
			convergenceCurve[iter] = optimalScore; //(4) se guarda el valor de la mejor aptitud encontrada hasta ahora. en convergenceCurve en 
                                                               //la posicion de la iteracion actaul (se actualiza optimalPos en el metodo calcFitness)

			// a decreases linearly from 2 to 0 in Eq. (2.3)
                        //a decrementa linealmente de 2 a 0 por cada iteracion.
			double a = 2.0 - (double) iter * (2.0 / maxIter);

			// a2 linearly decreases from -1 to -2 to calculate t in Eq. (3.12)
			double a2 = -1.0 + (double) iter * (-1.0 / maxIter);

			updatePosition(a, a2); //(3)actualiza posicion de la poblacion de ballenas
		}
		calcFitness(); //(4)evalua nuevamente la aptitud de las ballenas de la poblacion actual
